...
class Product(models.Model):
    ...
    suppliers = models.ManyToManyField('Supplier', related_name='supplied_products')
    ...
    def update_stock(self):
        if self.status == 'received':
            for item in self.items.all():
                order_item = OrderItem.objects.get(order=self, product=item)
                item.stock += order_item.quantity  # Mise à jour du stock
                item.save()
...
class Sale(models.Model):
    ...
    def __str__(self):
        return f'{self.quantity} unités de {self.product.name} vendues le {self.date_sold}'
